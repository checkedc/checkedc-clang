...
void BoundsWideningAnalysis::GetVarsToWiden(Expr *E, VarSetTy &VarsToWiden) {
  ...
  for (const VarDecl *V : VarsInExpr) {
    // Get the set of null-terminated arrays in whose bounds expressions V
    // occurs.
    // TODO: Actually, we need to gather the set of null-terminated arrays in
    // whose *upper* bounds expressions V occurs. But currently we do not store
    // whether a variable occurs in the lower or upper bounds expressions in
    // BoundsVars. As a result, currently we might end up widening even if a
    // variable does not appear in the upper bounds but appears in the lower
    // bounds of a null-terminated array. Also see TODO below.
    auto It = BoundsVars.find(V);

    // If V does not appear in the bounds expression of any null-terminated
    // array then expression E cannot potentially widen the bounds of any
  ...
  for (const VarDecl *V : CommonNullTermArrays) {
    // Gather the set of variables occurring in the bound expression of V.
    // TODO: Actually, we need to gather the set of variables occurring in the
    // *upper* bounds expression of V. But currently we do not store whether
    // a variable occurs in the lower or upper bounds expressions in
    // BoundsVars. As a result, currently we might end up widening even if a
    // variable does not appear in the upper bounds but appears in the lower
    // bounds of a null-terminated array. Also see TODO above.
    ...
    // while computing BoundsVars in CheckedCAnalysesPrepass.cpp.
    VarSetTy VarsInBounds;
    for (auto Pair : BoundsVars) {
