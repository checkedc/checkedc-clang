...
CVarSet
  ConstraintResolver::getOrCreatePersistentConstraints(clang::Expr *E,
                         llvm::function_ref<CVarSet (clang::Expr *)> ConsGen) {
  if (hasPersistentConstraints(E))
    return getPersistentConstraints(E);

  CVarSet Ret = ConsGen(E);
  storePersistentConstraints(E, Ret);
  return Ret;
}
...
CVarSet
  ...
  if (E != nullptr) {
    ...
    // (T)e
      ...
      auto ECEHandler = [this](Expr *E) {
        QualType TypE = E->getType();
        ExplicitCastExpr *ECE = dyn_cast<ExplicitCastExpr>(E);
      ...
      return getOrCreatePersistentConstraints(E, ECEHandler);
    // variable (x)
    ...
    // x = y, x+y, x+=y, etc.
      ...
      auto BOHandler = [this](Expr *E) {
        CVarSet EmptyCSet;
        auto TypE = E->getType();
        BinaryOperator *BO = dyn_cast<BinaryOperator>(E);
        assert (BO != nullptr && "Invalid handler called.");
        switch (BO->getOpcode()) {
      ...
      return getOrCreatePersistentConstraints(E, BOHandler);
    // x[e]
    } else if (isa<ArraySubscriptExpr>(E)) {
      auto ASEHandler = [this](Expr *E) {
        ArraySubscriptExpr *ASE = dyn_cast<ArraySubscriptExpr>(E);
        assert (ASE != nullptr && "Invalid handler called.");
        CVarSet T = getExprConstraintVars(ASE->getBase());
        CVarSet tmp = handleDeref(T);
        T.swap(tmp);
        return T;
      };
      return getOrCreatePersistentConstraints(E, ASEHandler);

    // ++e, &e, *e, etc.
    } else if (isa<UnaryOperator>(E)) {
      auto UOHandler = [this](Expr *E) {
        CVarSet EmptyCSet;
        auto TypE = E->getType();
        auto &CS = Info.getConstraints();
        UnaryOperator *UO = dyn_cast<UnaryOperator>(E);
        assert (UO != nullptr && "Invalid handler called.");
      ...
      return getOrCreatePersistentConstraints(E, UOHandler);
    // f(e1,e2, ...)
    } else if (isa<CallExpr>(E)) {
      auto CEHandler = [this](Expr *E) {
        auto &CS = Info.getConstraints();
        CallExpr *CE = dyn_cast<CallExpr>(E);
        assert (CE != nullptr && "Invalid handler called.");
    ...
    // { e1, e2, e3, ... }
      ...
      auto ILEHandler = [this](Expr *E) {
        ...
        if (ILE->getType()->isArrayType()) {
          ...
          // structure initialization should not reach here, as that caught by the non-pointer check at the top of this method.
      ...
      return getOrCreatePersistentConstraints(E, ILEHandler);
    // (int[]){e1, e2, e3, ... }
    } else if (isa<CompoundLiteralExpr>(E)) {
      auto CLEHandler = [this](Expr *E) {
        CompoundLiteralExpr *CLE = dyn_cast<CompoundLiteralExpr>(E);
        CVarSet T;

        CVarSet Vars =
            getExprConstraintVars(CLE->getInitializer());
      ...
      return getOrCreatePersistentConstraints(E, CLEHandler);

    // "foo"
    } else if (isa<clang::StringLiteral>(E)) {
      auto StrHandler = [this](Expr *E) {
        auto &CS = Info.getConstraints();
        CVarSet T;
        clang::StringLiteral *Str = dyn_cast<clang::StringLiteral>(E);
        assert (Str != nullptr && "Invalid handler called.");
      ...
      return getOrCreatePersistentConstraints(E, StrHandler);
    // Checked-C temporary
