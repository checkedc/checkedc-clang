...
int *f17(int *p) : byte_count(4) _Unchecked { // expected-note {{(expanded) declared return bounds are 'bounds((_Array_ptr<char>)_Return_value, (_Array_ptr<char>)_Return_value + 4)'}}
  return p; // expected-error {{return value has unknown bounds, bounds expected because the function 'f17' has bounds}}
}

int *f18(int *p : bounds(unknown)) : count(5) _Checked { // expected-note {{(expanded) declared return bounds are 'bounds(_Return_value, _Return_value + 5)'}}
  return p; // expected-error {{return value has unknown bounds, bounds expected because the function 'f18' has bounds}}
}
