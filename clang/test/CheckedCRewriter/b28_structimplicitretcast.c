...
//RUN: %clang -c %S/b28_structimplicitretcast.checkedNOALL.c
//RUN: rm %S/b28_structimplicitretcast.checkedNOALL.c
...
//CHECK_NOALL: _Ptr<struct r> sus(struct r x, struct r y) {
//CHECK_NOALL:   _Ptr<struct r> z =  malloc<struct r>(sizeof(struct r));
//CHECK_ALL: _Ptr<struct r> sus(struct r x, struct r y) {
