...
//RUN: %clang -c %S/b11_calleestructnp.checkedNOALL.c
//RUN: rm %S/b11_calleestructnp.checkedNOALL.c
...
//CHECK_NOALL: struct np * sus(struct p x, struct p y) {
//CHECK_NOALL:   struct np *z = malloc<struct np>(sizeof(struct np));
