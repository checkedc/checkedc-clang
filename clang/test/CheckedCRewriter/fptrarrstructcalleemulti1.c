// RUN: cconv-standalone -base-dir=%S -alltypes -output-postfix=checkedALL %s %S/fptrarrstructcalleemulti2.c
// RUN: cconv-standalone -base-dir=%S -output-postfix=checkedNOALL %s %S/fptrarrstructcalleemulti2.c
//RUN: %clang -c %S/fptrarrstructcalleemulti1.checkedNOALL.c %S/fptrarrstructcalleemulti2.checkedNOALL.c
//RUN: FileCheck -match-full-lines -check-prefixes="CHECK_NOALL" --input-file %S/fptrarrstructcalleemulti1.checkedNOALL.c %s
//RUN: FileCheck -match-full-lines -check-prefixes="CHECK_ALL" --input-file %S/fptrarrstructcalleemulti1.checkedALL.c %s
...
//CHECK_NOALL: struct fptrarr * sus(struct fptrarr *, struct fptrarr *y : itype(_Ptr<struct fptrarr>));
//CHECK_ALL: struct fptrarr * sus(struct fptrarr *, struct fptrarr *y : itype(_Ptr<struct fptrarr>));
