// RUN: cconv-standalone -base-dir=%S -alltypes -output-postfix=checkedALL %s %S/safefptrargcallermulti2.c
// RUN: cconv-standalone -base-dir=%S -output-postfix=checkedNOALL %s %S/safefptrargcallermulti2.c
//RUN: %clang -c %S/safefptrargcallermulti1.checkedNOALL.c %S/safefptrargcallermulti2.checkedNOALL.c
//RUN: FileCheck -match-full-lines -check-prefixes="CHECK_NOALL" --input-file %S/safefptrargcallermulti1.checkedNOALL.c %s
//RUN: FileCheck -match-full-lines -check-prefixes="CHECK_ALL" --input-file %S/safefptrargcallermulti1.checkedALL.c %s
...
//CHECK_ALL: int * sus(int (*)(int), _Ptr<int (int )> y);
