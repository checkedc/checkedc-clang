// RUN: cconv-standalone -base-dir=%S -alltypes -output-postfix=checked %s %S/fptrarrcalleemulti-alltypes2.c
//RUN: FileCheck -match-full-lines --input-file %S/fptrarrcalleemulti-alltypes1.checked.c %s
//RUN: rm %S/fptrarrcalleemulti-alltypes1.checked.c %S/fptrarrcalleemulti-alltypes2.checked.c
...
/*For robustness, this test is identical to fptrarrprotocallee-alltypes.c and fptrarrcallee-alltypes.c except in that
...
int ** sus(int *, int *);
//CHECK: int ** sus(int *, int *y : itype(_Array_ptr<int>));
