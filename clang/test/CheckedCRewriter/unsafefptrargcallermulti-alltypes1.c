// RUN: cconv-standalone -base-dir=%S -alltypes -output-postfix=checked %s %S/unsafefptrargcallermulti-alltypes2.c
//RUN: FileCheck -match-full-lines --input-file %S/unsafefptrargcallermulti-alltypes1.checked.c %s
//RUN: rm %S/unsafefptrargcallermulti-alltypes1.checked.c %S/unsafefptrargcallermulti-alltypes2.checked.c
...
/*For robustness, this test is identical to unsafefptrargprotocaller-alltypes.c and unsafefptrargcaller-alltypes.c except in that
