...
// RUN: 3c -base-dir=%t.checked -alltypes %t.checked/b30_structprotocastunsafeexplicit.c -- | diff %t.checked/b30_structprotocastunsafeexplicit.c -
#include <stddef.h>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
struct np *bar() {
  ...
  //CHECK: struct np *z = (struct np *)sus(_Assume_bounds_cast<_Ptr<struct r>>(x), _Assume_bounds_cast<_Ptr<struct r>>(y));
  return z;
}
