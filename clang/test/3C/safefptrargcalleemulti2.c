// RUN: 3c -base-dir=%S -addcr -alltypes -output-postfix=checkedALL2 %S/safefptrargcalleemulti1.c %s
// RUN: 3c -base-dir=%S -addcr -output-postfix=checkedNOALL2 %S/safefptrargcalleemulti1.c %s
// RUN: %clang -c %S/safefptrargcalleemulti1.checkedNOALL2.c %S/safefptrargcalleemulti2.checkedNOALL2.c
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_NOALL","CHECK" --input-file %S/safefptrargcalleemulti2.checkedNOALL2.c %s
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_ALL","CHECK" --input-file %S/safefptrargcalleemulti2.checkedALL2.c %s
// RUN: 3c -base-dir=%S -alltypes -output-postfix=checked2 %S/safefptrargcalleemulti1.c %s
// RUN: 3c -base-dir=%S -alltypes -output-postfix=convert_again %S/safefptrargcalleemulti1.checked2.c %S/safefptrargcalleemulti2.checked2.c
...
/*For robustness, this test is identical to safefptrargprotocallee.c and safefptrargcallee.c except in that
