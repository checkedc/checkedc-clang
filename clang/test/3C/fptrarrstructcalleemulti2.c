...
// RUN: %clang -working-directory=%t.checkedNOALL2 -c fptrarrstructcalleemulti1.c fptrarrstructcalleemulti2.c
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_NOALL","CHECK" --input-file %t.checkedNOALL2/fptrarrstructcalleemulti2.c %s
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_ALL","CHECK" --input-file %t.checkedALL2/fptrarrstructcalleemulti2.c %s
// RUN: 3c -base-dir=%S -alltypes -output-dir=%t.checked %S/fptrarrstructcalleemulti1.c %s --
...
fptrarrstructprotocallee.c and fptrarrstructcallee.c except in that
