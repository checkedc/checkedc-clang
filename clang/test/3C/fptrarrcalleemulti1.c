// RUN: rm -rf %t*
// RUN: 3c -base-dir=%S -addcr -alltypes -output-dir=%t.checkedALL %s %S/fptrarrcalleemulti2.c --
// RUN: 3c -base-dir=%S -addcr -output-dir=%t.checkedNOALL %s %S/fptrarrcalleemulti2.c --
// RUN: %clang -working-directory=%t.checkedNOALL -c fptrarrcalleemulti1.c fptrarrcalleemulti2.c
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_NOALL","CHECK" --input-file %t.checkedNOALL/fptrarrcalleemulti1.c %s
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_ALL","CHECK" --input-file %t.checkedALL/fptrarrcalleemulti1.c %s
// RUN: 3c -base-dir=%S -alltypes -output-dir=%t.checked %S/fptrarrcalleemulti2.c %s --
// RUN: 3c -base-dir=%t.checked -alltypes -output-dir=%t.convert_again %t.checked/fptrarrcalleemulti1.c %t.checked/fptrarrcalleemulti2.c --
// RUN: test ! -f %t.convert_again/fptrarrcalleemulti1.c
// RUN: test ! -f %t.convert_again/fptrarrcalleemulti2.c
...
/*For robustness, this test is identical to fptrarrprotocallee.c and fptrarrcallee.c except in that
