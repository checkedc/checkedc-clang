// RUN: 3c -base-dir=%S -alltypes -use-malloc=my_malloc,your_malloc %s -- | FileCheck -match-full-lines -check-prefixes="CHECK_ALL","CHECK" %s
// RUN: 3c -base-dir=%S -use-malloc=my_malloc,your_malloc %s -- | FileCheck -match-full-lines -check-prefixes="CHECK_NOALL","CHECK" %s
