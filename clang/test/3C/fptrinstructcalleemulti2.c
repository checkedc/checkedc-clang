// RUN: rm -rf %t*
// RUN: 3c -base-dir=%S -addcr -alltypes -output-dir=%t.checkedALL2 %S/fptrinstructcalleemulti1.c %s --
// RUN: 3c -base-dir=%S -addcr -output-dir=%t.checkedNOALL2 %S/fptrinstructcalleemulti1.c %s --
// RUN: %clang -working-directory=%t.checkedNOALL2 -c fptrinstructcalleemulti1.c fptrinstructcalleemulti2.c
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_NOALL","CHECK" --input-file %t.checkedNOALL2/fptrinstructcalleemulti2.c %s
// RUN: FileCheck -match-full-lines -check-prefixes="CHECK_ALL","CHECK" --input-file %t.checkedALL2/fptrinstructcalleemulti2.c %s
// RUN: 3c -base-dir=%S -alltypes -output-dir=%t.checked %S/fptrinstructcalleemulti1.c %s --
// RUN: 3c -base-dir=%t.checked -alltypes -output-dir=%t.convert_again %t.checked/fptrinstructcalleemulti1.c %t.checked/fptrinstructcalleemulti2.c --
// RUN: test ! -f %t.convert_again/fptrinstructcalleemulti1.c
// RUN: test ! -f %t.convert_again/fptrinstructcalleemulti2.c
...
fptrinstructprotocallee.c and fptrinstructcallee.c except in that
