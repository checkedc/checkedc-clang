...
We have two operators for temporary variables: binding and use.

The `CHCKBindTempoaryExpr` binds a temporary variable to the
...
as other C expression.

The `BoundsValueExpr` reads the value of a temporary variable.
Its constructor takes a `CHKCBindTemporaryExpr` as an
argument.  We do not create a variable name for the
temporary expression.  The name used during error message
is `temporary variable for result of e`, where `e` is the
expression whose result is being computed.

Temporary variables are introduced for string and array
literals.  They are also introduced for call expressions
whose return bounds use the `_Return_value` expression.
...
for Checked C bounds declaration checking and those
...
to wryr about. For Checked C, these temporaries are tracked and
rename in the fragment of the AST being transfored.  Therefore these
variables should not be regenerated by semantic actions invoked by
TreeTransform. The Checked C temporaries shold be introduced during the initial
