...
"%MSBUILD_BIN%" projects\checkedc-wrapper\check-checkedc.vcxproj /v:%MSBUILD_VERBOSITY% /maxcpucount:%MSBUILD_CPU_COUNT%
if ERRORLEVEL 1 (goto cmdfailed)

if "%TEST_SUITE%"=="CheckedC" (
  rem
) else if "%TEST_SUITE%"=="CheckedC_clang" (
  "%MSBUILD_BIN%" tools\clang\test\check-clang.vcxproj /v:%MSBUILD_VERBOSITY% /maxcpucount:%MSBUILD_CPU_COUNT%
  if ERRORLEVEL 1 (goto cmdfailed)
) else if "%TEST_SUITE%"=="CheckedC_LLVM" (
  "%MSBUILD_BIN%" check-all.vcxproj /v:%MSBUILD_VERBOSITY% /maxcpucount:%MSBUILD_CPU_COUNT%
  if ERRORLEVEL 1 (goto cmdfailed)
)
