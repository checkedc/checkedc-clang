...
static bool isAtLeastAsChecked(bool Self, bool Other) {
    return (Self || Self == Other);
}

bool QualType::isAtLeastAsCheckedAs(QualType Other) const {
  QualType Self = *this;

  if (Self.isNull() || Other.isNull())
    return true;

  if (Self.getQualifiers() != Other.getQualifiers())
    return false;

  Self = getCanonicalType();
  Other = Other.getCanonicalType();

  const Type *SelfType = Self.getTypePtr();
  const Type *OtherType = Other.getTypePtr();

  if (SelfType == OtherType)
    return true;

   Type::TypeClass SelfTypeClass = Self->getTypeClass();
  if (SelfTypeClass != OtherType->getTypeClass())
    return false;

  switch (SelfTypeClass) {
    case Type::Pointer: {
      const PointerType *SelfPtrType = cast<PointerType>(SelfType);
      const PointerType *OtherPtrType = cast<PointerType>(OtherType);
      if (!isAtLeastAsChecked(SelfPtrType->isChecked(),
                             OtherPtrType->isChecked()))
        return false;

      QualType SelfPointeeType = SelfPtrType->getPointeeType();
      QualType OtherPointeeType = OtherPtrType->getPointeeType();
      if (!SelfPointeeType.isAtLeastAsCheckedAs(OtherPointeeType)) {
        return false;
      }
    ...
    case Type::IncompleteArray: {
      ...
      const ArrayType *SelfArrayType = cast<ArrayType>(SelfType);
      const ArrayType *OtherArrayType = cast<ArrayType>(OtherType);
      if (SelfArrayType->getSizeModifier() != OtherArrayType->getSizeModifier())
        return false;

      if (!isAtLeastAsChecked(SelfArrayType->isChecked(),
                             OtherArrayType->isChecked()))
        return false;


      QualType SelfElementType = SelfArrayType->getElementType();
      QualType OtherElementType = OtherArrayType->getElementType();
      if (!SelfElementType.isAtLeastAsCheckedAs(OtherElementType))
        return false;

        // check properties for specific kinds of arrays
      if (SelfTypeClass == Type::ConstantArray) {
        const ConstantArrayType *SelfConstantArrayType =
          cast<ConstantArrayType>(SelfType);
        const ConstantArrayType *OtherConstantArrayType =
          cast<ConstantArrayType>(OtherType);
        if (!llvm::APInt::isSameValue(SelfConstantArrayType->getSize(),
                                      OtherConstantArrayType->getSize()))
          return false;
      }
    ...
    case Type::FunctionProto: {
      const FunctionProtoType *SelfFuncType =
        cast<FunctionProtoType>(SelfType);
      const FunctionProtoType *OtherFuncType =
        cast<FunctionProtoType>(OtherType);

      // Check return types
      QualType SelfReturnType = SelfFuncType->getReturnType();
      QualType OtherReturnType = OtherFuncType->getReturnType();
      if (!SelfReturnType.isAtLeastAsCheckedAs(OtherReturnType))
        return false;

      // Check parameter types and parameter-specific information.
      unsigned int paramCount = SelfFuncType->getNumParams();
      if (paramCount != OtherFuncType->getNumParams())
        return false;

      for (unsigned int i = 0; i < paramCount; i++) {
        QualType SelfParamType = SelfFuncType->getParamType(i);
        QualType OtherParamType = OtherFuncType->getParamType(i);
        if (!SelfParamType.isAtLeastAsCheckedAs(OtherParamType))
          return false;
        // check additional non-type information about parameters
        FunctionProtoType::ExtParameterInfo SelfInfo =
          SelfFuncType->getExtParameterInfo(i);
        FunctionProtoType::ExtParameterInfo OtherInfo =
          OtherFuncType->getExtParameterInfo(i);
        if (SelfInfo != OtherInfo)
          return false;
      }

      // Check other properties of the function type.
      if (SelfFuncType->getExtInfo() != OtherFuncType->getExtInfo())
        return false;

      if (SelfFuncType->isVariadic() != OtherFuncType->isVariadic())
        return false;
...
bool QualType::isEqualIgnoringChecked(QualType Other) const {
  QualType Self = *this;
  ...
  case Type::Pointer: {
    ...
    QualType SelfPointeeType = SelfPtrType->getPointeeType();
    QualType OtherPointeeType = OtherPtrType->getPointeeType();
    if (!SelfPointeeType.isEqualIgnoringChecked(OtherPointeeType)) {
      return false;
    }
  ...
  case Type::IncompleteArray: {
    ...
    QualType SelfElementType = SelfArrayType->getElementType();
    QualType OtherElementType = OtherArrayType->getElementType();
    if (!SelfElementType.isEqualIgnoringChecked(OtherElementType))
      return false;
  ...
  case Type::FunctionProto: {
    const FunctionProtoType *SelfFuncType = cast<FunctionProtoType>(SelfType);
    const FunctionProtoType *OtherFuncType =
      cast<FunctionProtoType>(OtherType);
    ...
    QualType SelfReturnType = SelfFuncType->getReturnType();
    QualType OtherReturnType = OtherFuncType->getReturnType();
    if (!SelfReturnType.isEqualIgnoringChecked(OtherReturnType))
      return false;
    ...
    for (unsigned int i = 0; i < paramCount; i++) {
      QualType SelfParamType = SelfFuncType->getParamType(i);
      QualType OtherParamType = OtherFuncType->getParamType(i);
      if (!SelfParamType.isEqualIgnoringChecked(OtherParamType))
        return false;
    }
