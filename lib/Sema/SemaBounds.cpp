...
//    information in determining if two expressions are the same.  Users of
//    bounds expressions that have been abstracted need to be aware that line
//    number information may be inaccurate.
...
public:
  ...
  ExprResult TransformDeclRefExpr(DeclRefExpr *E) {
        ...
        for (auto &ParamInfo : Params)
          if (PD == ParamInfo.Param) {
            return SemaRef.CreatePositionalParameterExpr(
                PD->getFunctionScopeIndex(), PD->getType());
          }
        SemaRef.Diag(E->getLocation(),
    ...
    ValueDecl *ND = dyn_cast_or_null<ValueDecl>(
        BaseTransform::TransformDecl(SourceLocation(), D));
    if (D == ND || ND == nullptr)
      return E;
    else {
...
bool Sema::AbstractForFunctionType(
    BoundsAnnotations &Annots, ArrayRef<DeclaratorChunk::ParamInfo> Params) {
...
public:
  ConcretizeBoundsExpr(Sema &SemaRef, ArrayRef<ParmVarDecl *> Params)
      : BaseTransform(SemaRef), Parameters(Params) {}
  ...
  ExprResult TransformPositionalParameterExpr(PositionalParameterExpr *E) {
    ...
    if (index < Parameters.size()) {
      ...
      return SemaRef.BuildDeclRefExpr(
          PD, E->getType(), clang::ExprValueKind::VK_LValue, SourceLocation());
    } else {
...
BoundsExpr *Sema::ConcretizeFromFunctionType(BoundsExpr *Expr,
  ...
  ExprResult ConcreteBounds =
      ConcretizeBoundsExpr(*this, Params).TransformExpr(Expr);
...
namespace {
class CheckForModifyingArgs
    : public RecursiveASTVisitor<CheckForModifyingArgs> {
private:
  Sema &SemaRef;
...
public:
  CheckForModifyingArgs(Sema &SemaRef, ArrayRef<Expr *> Args,
                        Sema::NonModifyingContext ErrorKind)
      : SemaRef(SemaRef), Arguments(Args), VisitedArgs(Args.size()),
        ErrorKind(ErrorKind), ModifyingArg(false) {}

  bool FoundModifyingArg() { return ModifyingArg; }
...
namespace {
class ConcretizeBoundsExprWithArgs
    : public TreeTransform<ConcretizeBoundsExprWithArgs> {
...
public:
  ConcretizeBoundsExprWithArgs(Sema &SemaRef, ArrayRef<Expr *> Args)
      : BaseTransform(SemaRef), Args(Args) {}
...
BoundsExpr *Sema::ConcretizeFromFunctionTypeWithArgs(
    BoundsExpr *Bounds, ArrayRef<Expr *> Args, NonModifyingContext ErrorKind) {
...
#endif
    llvm_unreachable(
        "unexpected failure in making function bounds concrete with arguments");
    return nullptr;
  } else {
  ...
  ExprResult TransformDeclRefExpr(DeclRefExpr *E) {
    if (FieldDecl *FD = dyn_cast<FieldDecl>(E->getDecl())) {
      ...
      MemberExpr *ME = new (Context)
          MemberExpr(Base, IsArrow, SourceLocation(), FD, SourceLocation(),
...
BoundsExpr *Sema::MakeMemberBoundsConcrete(Expr *Base, bool IsArrow,
                                           BoundsExpr *Bounds) {
  ExprSubstitutionScope Scope(*this); // suppress diagnostics
  ExprResult ConcreteBounds =
...
private:
  ...
  BoundsExpr *CreateBoundsAllowedButNotComputed() { return CreateBoundsAny(); }
  ...
  BoundsExpr *CreateBoundsNotAllowedYet() { return CreateBoundsUnknown(); }
...
public:
  ImplicitCastExpr *CreateImplicitCast(QualType Target, CastKind CK, Expr *E) {
  ...
  Expr *CreateExplicitCast(QualType Target, CastKind CK, Expr *E,
    ...
    CStyleCastExpr *CE =
        CStyleCastExpr::Create(Context, Target, ExprValueKind::VK_RValue, CK, E,
                               nullptr, DI, SourceLocation(), SourceLocation());
...
private:
  Expr *CreateAddressOfOperator(Expr *E) {
    QualType Ty =
        Context.getPointerType(E->getType(), CheckedPointerKind::Array);
    return new (Context) UnaryOperator(
        E, UnaryOperatorKind::UO_AddrOf, Ty, ExprValueKind::VK_RValue,
  ...
  IntegerLiteral *CreateIntegerLiteral(const llvm::APInt &I) {
    ...
    IntegerLiteral *Lit =
        IntegerLiteral::Create(Context, ResultVal, Ty, SourceLocation());
    return Lit;
  }
...
public:
  ...
  BoundsExpr *CreateBoundsForArrayType(QualType QT) {
    ...
    CountBoundsExpr *CBE =
        new (Context) CountBoundsExpr(BoundsExpr::Kind::ElementCount, Size,
                                      SourceLocation(), SourceLocation());
    return CBE;
  }

  // Only used for doing CreatBounds + 1, will be used in bounds widening
  // for Nt_array_ptr (RangeBoundsExpr(lower, upper+1))
  BoundsExpr *CreateBoundsPlusOneForNtArray(QualType QT) {
    const IncompleteArrayType *IAT = Context.getAsIncompleteArrayType(QT);
    if (IAT) {
      if (IAT->getKind() == CheckedArrayKind::NtChecked)
        return Context.getPrebuiltCountZero();
    ...
    IntegerLiteral *Size = CreateIntegerLiteral(size + 1);
    CountBoundsExpr *CBE =
        new (Context) CountBoundsExpr(BoundsExpr::Kind::ElementCount, Size,
                                      SourceLocation(), SourceLocation());
    return CBE;
  ...
  BoundsExpr *ExpandToRange(Expr *Base, BoundsExpr *B) {
    ...
    case BoundsExpr::Kind::ElementCount: {
      ...
      if (K == BoundsExpr::ByteCount) {
        ResultTy =
            Context.getPointerType(Context.CharTy, CheckedPointerKind::Array);
      ...
      Expr *UpperBound = new (Context) BinaryOperator(
          LowerBound, Count, BinaryOperatorKind::BO_Add, ResultTy,
          ExprValueKind::VK_RValue, ExprObjectKind::OK_Ordinary,
          SourceLocation(), FPOptions());
      RangeBoundsExpr *R = new (Context) RangeBoundsExpr(
...
public:
  BoundsInference(Sema &S, bool IncludeNullTerminator = false)
      ...
      if (ME->getType()->isArrayType()) {
        ...
        if (B) {
          ...
          if (B->isElementCount() || B->isByteCount()) {
            Expr *Base =
                CreateImplicitCast(Context.getDecayedType(E->getType()),
                                   CastKind::CK_ArrayToPointerDecay, E);
            return ExpandToRange(Base, B);
          } else
            return B;
  ...
  BoundsExpr *CreateTypeBasedBounds(Expr *E, QualType Ty, bool IsParam,
    ...
    if (Base->isLValue())
      Base =
          CreateImplicitCast(E->getType(), CastKind::CK_LValueToRValue, Base);

    // If type is a bounds-safe interface type, adjust the type of base to the
    // bounds-safe interface type.
    if (IsBoundsSafeInterface) {
      // Compute the target type.  We could receive an array type for a
      // parameter with a bounds-safe interface.
  ...
  BoundsExpr *LValueTargetBounds(Expr *E) {
    ...
    case Expr::MemberExprClass: {
      ...
      if (!B) {
        // This can happen when MemberBaseExpr is an rvalue expression.  An
        // example of this a function call that returns a struct.
        // MakeMemberBoundsConcrete can't handle this yet.
  ...
  BoundsExpr *RValueBounds(Expr *E) {
    ...
    case Expr::BoundsCastExprClass: {
      ...
      Expr *subExprAtNewType =
          CreateExplicitCast(E->getType(), CastKind::CK_BitCast, subExpr, true);
    ...
    case Expr::UnaryOperatorClass: {
      ...
      if (Op == UnaryOperatorKind::UO_Deref) {
        llvm_unreachable(
            "unexpected dereference expression in RValue Bounds inference");
        return CreateBoundsInferenceError();
      }
      ...
      if (Op == UnaryOperatorKind::UO_Plus ||
          Op == UnaryOperatorKind::UO_Minus || Op == UnaryOperatorKind::UO_Not)
    ...
    case Expr::CompoundAssignOperatorClass: {
      ...
      // `p += i` has the lvalue target bounds of `p`. `p` is an LValue. `p +=
      // i` is an RValue same applies for `-` and `-=` respectively
      if (LHS->getType()->isPointerType() && RHS->getType()->isIntegerType() &&
          BinaryOperator::isAdditiveOp(Op)) {
        return IsCompoundAssignment ? LValueTargetBounds(LHS)
                                    : RValueBounds(LHS);
      }
      ...
      // `e -= p` has empty bounds, regardless of the bounds of p.
      if (RHS->getType()->isPointerType() && Op == BinaryOperatorKind::BO_Sub) {
        return CreateBoundsEmpty();
      }
      ...
      if (LHS->getType()->isIntegerType() && RHS->getType()->isIntegerType() &&
          (BinaryOperator::isAdditiveOp(Op) ||
           ...
           BinaryOperator::isBitwiseOp(Op) || BinaryOperator::isShiftOp(Op))) {
        BoundsExpr *LHSBounds =
            IsCompoundAssignment ? LValueTargetBounds(LHS) : RValueBounds(LHS);
    ...
    case Expr::CallExprClass: {
      ...
      if (E->getType()->isCheckedPointerPtrType()) {
        ...
        ArrayRef<Expr *> ArgExprs = llvm::makeArrayRef(
            const_cast<Expr **>(CE->getArgs()), CE->getNumArgs());

        // Concretize Call Bounds with argument expressions.
        // We can only do this if the argument expressions are non-modifying
        ReturnBounds = SemaRef.ConcretizeFromFunctionTypeWithArgs(
            FunBounds, ArgExprs,
      ...
      // general case.
      if (ReturnBounds->isElementCount() || ReturnBounds->isByteCount())
        return CreateBoundsAllowedButNotComputed();
...
BoundsExpr *Sema::ExpandToRange(VarDecl *D, BoundsExpr *B) {
  ...
  ExprResult ER = BuildDeclRefExpr(D, QT, clang::ExprValueKind::VK_LValue,
                                   SourceLocation());
  if (ER.isInvalid())
    return nullptr;
  Expr *Base = ER.get();
...
Expr *Sema::MakeAssignmentImplicitCastExplicit(Expr *E) {
  ...
  return BoundsInference(*this).CreateExplicitCast(
      TargetTy, CK, SE, ICE->isBoundsSafeInterface());
}
...
private:
  ...
  void DumpBoundsCastBounds(raw_ostream &OS, CastExpr *E, BoundsExpr *Declared,
                            BoundsExpr *NormalizedDeclared,
  ...
  void DumpInitializerBounds(raw_ostream &OS, VarDecl *D, BoundsExpr *Target,
                             BoundsExpr *B) {
    OS << "\n";
    D->dump(OS);
    OS << "Declared Bounds:\n";
  ...
  //
  // This enum is used in generating diagnostic messages. If you change the
  // order, update the messages in DiagnosticSemaKinds.td used in
  // ExplainProofFailure
  ...
  enum class ProofFailure : unsigned {
    None = 0x0,
    LowerBound =
        0x1, // The destination lower bound is below the source lower bound.
    UpperBound =
        0x2,     // The destination upper bound is above the source upper bound.
    Empty = 0x4, // The source bounds are empty.
    Width = 0x8, // The source bounds are narrower than the destination bounds.
    PartialOverlap =
        0x16 // There was only partial overlap of the destination bounds with
             // the source bounds.
  };

  enum class DiagnosticNameForTarget { Destination = 0x0, Target = 0x1 };
  ...
  // is an expression.  For now, we represent const1 and const2 as signed
  // (APSInt) integers.  They must have the same bitsize.
  ...
  public:
    ConstantSizedRange(Sema &S)
        : S(S), Base(nullptr), LowerOffset(1, true), UpperOffset(1, true) {}

    ConstantSizedRange(Sema &S, Expr *Base, llvm::APSInt &LowerOffset,
                       llvm::APSInt &UpperOffset)
    ...
    bool IsEmpty() { return UpperOffset <= LowerOffset; }
    ...
    ProofResult PartialOverlap(ConstantSizedRange &R) {
      if (Lexicographic(S.Context, nullptr).CompareExpr(Base, R.Base) ==
        ...
        if (!IsEmpty() && !R.IsEmpty()) {
          // R.LowerOffset is within this range, but R.UpperOffset is above the
          // range
          if (LowerOffset <= R.LowerOffset && R.LowerOffset < UpperOffset &&
              UpperOffset < R.UpperOffset)
            return ProofResult::True;
          // Or R.UpperOffset is within this range, but R.LowerOffset is below
          // the range.
    ...
    llvm::APSInt GetWidth() { return UpperOffset - LowerOffset; }

    void SetBase(Expr *B) { Base = B; }

    void SetLower(llvm::APSInt &Lower) { LowerOffset = Lower; }

    void SetUpper(llvm::APSInt &Upper) { UpperOffset = Upper; }
  ...
  bool getReferentSizeInChars(QualType Ty, llvm::APSInt &Size) {
    ...
    uint64_t ElemSize =
        S.Context.toCharUnitsFromBits(ElemBitSize).getQuantity();
  ...
  void SplitIntoBaseAndOffset(Expr *E, Expr *&Base, llvm::APSInt &Offset) {
      ...
      if (BO->isAdditiveOp()) {
        ...
        if (Other->isIntegerConstantExpr(Offset, S.Context)) {
          ...
          if (BO->getOpcode() == BO_Sub) {
            Offset =
                llvm::APSInt(PointerWidth, false).ssub_ov(Offset, Overflow);
            if (Overflow)
              goto exit;
          }
  ...
  static bool EqualValue(ASTContext &Ctx, Expr *E1, Expr *E2,
    ...
    Lexicographic::Result R =
        Lexicographic(Ctx, EquivExprs).CompareExpr(E1, E2);
  ...
  // Try to prove that PtrBase + Offset is within Bounds, where PtrBase has
  // pointer type. Offset is optional and may be a nullptr.
  ProofResult ProveMemoryAccessInRange(Expr *PtrBase, Expr *Offset,
                                       BoundsExpr *Bounds, BoundsCheckKind Kind,
                                       ProofFailure &Cause) {
#ifdef TRACE_RANGE
    llvm::outs() << "Examining:\nPtrBase\n";
    ...
    Bounds->dump(llvm::outs());
#endif
    assert(BoundsUtil::IsStandardForm(Bounds) && "bounds not in standard form");
    Cause = ProofFailure::None;
  ...
  void ExplainProofFailure(SourceLocation Loc, ProofFailure Cause,
    ...
    if (Kind == ProofStmtKind::MemoryAccess ||
        Kind == ProofStmtKind::MemberArrowBase) {
  ...
  void CheckBoundsDeclAtAssignment(SourceLocation ExprLoc, Expr *Target,
                                   ...
                                   BoundsExpr *SrcBounds, bool InCheckedScope) {
    ...
    if (S.CheckIsNonModifying(Target, Sema::NonModifyingContext::NMC_Unknown,
        ...
        S.CheckIsNonModifying(Src, Sema::NonModifyingContext::NMC_Unknown,
                              Sema::NonModifyingMessage::NMM_None)) {
      Expr *TargetExpr = BoundsInference(S).CreateImplicitCast(
          Target->getType(), CK_LValueToRValue, Target);
  ...
  void CheckBoundsDeclAtCallArg(unsigned ParamNum,
                                ...
                                BoundsExpr *ArgBounds, bool InCheckedScope) {
    SourceLocation ArgLoc = Arg->getLocStart();
    ProofFailure Cause;
    ProofResult Result =
        ProveBoundsDeclValidity(ExpectedArgBounds, ArgBounds, Cause, nullptr);
    if (Result != ProofResult::True) {
      unsigned DiagId =
          (Result == ProofResult::False)
  ...
  void CheckBoundsDeclAtInitializer(SourceLocation ExprLoc, VarDecl *D,
    ...
    if (S.CheckIsNonModifying(Src, Sema::NonModifyingContext::NMC_Unknown,
      ...
      DeclRefExpr *TargetDeclRef = DeclRefExpr::Create(
          S.getASTContext(), NestedNameSpecifierLoc(), SourceLocation(), D,
          false, SourceLocation(), D->getType(), ExprValueKind::VK_LValue);
      CastKind Kind;
      QualType TargetTy;
      ...
      Expr *TargetExpr =
          BoundsInference(S).CreateImplicitCast(TargetTy, Kind, TargetDeclRef);
      EqualExpr.push_back(TargetExpr);
      EqualExpr.push_back(Src);
      EquivExprs.push_back(&EqualExpr);
      /*
  ...
  // the TargetBounds.
  void CheckBoundsDeclAtStaticPtrCast(CastExpr *Cast, BoundsExpr *TargetBounds,
    ...
    if (Result != ProofResult::True) {
      ...
      if (Result == ProofResult::False)
        ExplainProofFailure(ExprLoc, Cause, ProofStmtKind::StaticBoundsCast);
  ...
  void CheckBoundsAtMemoryAccess(Expr *Deref, BoundsExpr *ValidRange,
    ...
    if (UnaryOperator *UO = dyn_cast<UnaryOperator>(Deref)) {
      ...
      ProofKind = ProofStmtKind::MemoryAccess;
      Result = ProveMemoryAccessInRange(AS->getBase(), AS->getIdx(), ValidRange,
                                        CheckKind, Cause);
    } else if (MemberExpr *ME = dyn_cast<MemberExpr>(Deref)) {
      assert(ME->isArrow());
      ProofKind = ProofStmtKind::MemberArrowBase;
      Result = ProveMemoryAccessInRange(ME->getBase(), nullptr, ValidRange,
                                        CheckKind, Cause);
    } else {
      llvm_unreachable("unexpected expression kind");
    }
...
public:
  CheckBoundsDeclarations(Sema &SemaRef, Stmt *Body, CFG *Cfg,
                          BoundsExpr *ReturnBounds)
      : S(SemaRef), DumpBounds(SemaRef.getLangOpts().DumpInferredBounds),
        PointerWidth(SemaRef.Context.getTargetInfo().getPointerWidth(0)),
        Body(Body), Cfg(Cfg), ReturnBounds(ReturnBounds) {}

  typedef llvm::SmallPtrSet<const Stmt *, 16> StmtSet;
  typedef std::map<VarDecl *, BoundsExpr *> WidenedBoundsMap;
  ...
  //     1: x = (cond == 0) ? f1 : f2();
  //
  // For now, we want to skip B1.1, B2.1, and B3.1 because they will be
  // processed as part of B4.1.
  ...
  // check two exprs equal or not
  bool isIdenticalExpr(const Expr *Expr1, const Expr *Expr2) {

    bool isEqual = false;
    // we only care about non-empty expr, compare two nullptrs is meaningless
    if (Expr1 && Expr2) {
      ...
      while (I1 != Expr1->child_end() && I2 != Expr2->child_end()) {
        if (!*I1 || !*I2 ||
            !isIdenticalExpr(dyn_cast<Expr>(*I1), dyn_cast<Expr>(*I2)))
          return false;
        ++I1;
        ++I2;
      }
  ...
  void DataFlowAnalysisOnCFG(FunctionDecl *FD) {
    assert(Cfg && "expected CFG to exist");

    // for debugging only
    llvm::outs() << "------------- Dumping AST BEGIN----------------\n";
    Body->dump(llvm::outs());
    llvm::outs() << "------------- Dumping AST END ----------------\n";

    // Step 1: construct a list VarBounds of bounds of vardecals for variables
    // with nt_array_ptr, from the VarDecl*, we can get the bounds declaration
    // for the declaration
    list<VarDecl *> VarBounds;

    // walk the function parameter list to collect paramVarDecl with
    // nt_array_ptr type
    for (unsigned i = 0; i < FD->getNumParams(); i++) {
      ParmVarDecl *PD = FD->getParamDecl(i);
      llvm::outs() << "paramVarDecl: " << *PD << "\n";

      QualType paramTy = PD->getType();
      if (paramTy->isCheckedPointerNtArrayType()) {
        llvm::outs() << "God you we found a Nt_array_ptr! It's ";
        llvm::outs() << "paramVarDecl(*PD): " << *PD << " PD: " << PD << "\n";
        VarBounds.insert(VarBounds.begin(), PD);
      }
    }

    llvm::outs() << "------------------------ BEGIN "
                    "------------------------------------\n";
    // walk over each CFG block to find those VarDecl* DeclStmt, add them to the
    // list
    for (const CFGBlock *Block : *Cfg) {
      for (CFGElement Elem : *Block) {
        CFGStmt CS = Elem.castAs<CFGStmt>();
        ...
        if (DeclStmt *DS = dyn_cast<DeclStmt>(S)) {
          llvm::outs() << "This Stmt is an DeclStmt...\n";
          // Can we use getSingleDecl() here?
          for (auto I = DS->decl_begin(); I != DS->decl_end(); ++I) {
            Decl *D = *I;
            if (VarDecl *VD = dyn_cast<VarDecl>(D)) {
              llvm::outs() << "DeclStmt is a VarDecl: " << *VD << "\n";
              VarBounds.insert(VarBounds.begin(), VD);
            }
          }
        }
      }
    } // end walking over CFGBlock

    llvm::outs() << "VarBounds size: " << VarBounds.size() << "\n";

    // Step 2: if VarBounds is empty, the algorithm is finished
    if (VarBounds.empty()) {
      llvm::outs()
          << "There is no nt_array_ptr variables with bounds to widen!\n";
      return;
    }
    ...
    // Step 3: a map list the inferred "widened bounds" for nt_array_ptr
    // variables for each point in the program (points here are CFGElements)
    WidenedBoundsMap VarBoundsMap;

    llvm::outs() << "Before Step 4&5... \n ";

    // Step 4&5: for each CFGBlock, we have
    // an input  WidenedBoundSMap at the beginning of the basic block
    // an output WidenedBoundsmap for each edge exiting the basic block
    // the input/output maps should be allocated "on demand"
    // an empty map will be represented by a nullptr
    vector<WidenedBoundsMap *> inBoundsMap;
    vector<vector<WidenedBoundsMap *>> outBoundsMap;

    for (unsigned i = 0; i < Cfg->getNumBlockIDs(); i++) {
      inBoundsMap.push_back(nullptr);
      outBoundsMap.push_back(vector<WidenedBoundsMap *>());
    }
    llvm::outs() << "inBoundsMap.size : " << inBoundsMap.size() << "\n";
    llvm::outs() << "outBoundsMap.size: " << outBoundsMap.size() << "\n";

    llvm::outs() << "Before Step 7... \n ";
    // Step 7: For each CFGBlock(index - block ID), compute the set
    // "Modified" of variables assigned to by elements of this basic
    // block, here modified means:
    // (1) must be an l-value
    // (2) has DeclRefExpr
    // TODO: add test case for testing this snippet
    vector<set<VarDecl *>> Modified;
    for (const CFGBlock *Block : *Cfg) {
      for (CFGElement Elem : *Block) {
        CFGStmt CS = Elem.castAs<CFGStmt>();
        Stmt *ST = const_cast<Stmt *>(CS.getStmt());
        Expr *E = dyn_cast<Expr>(ST);
        // TODO: use RecursiveASTVisitor to handle general cases
        // for now start from processing BinaryOperator case first
        if (E != nullptr && isa<BinaryOperator>(E)) {
          BinaryOperator *BO = dyn_cast<BinaryOperator>(E);
          if (DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(BO->getLHS())) {
            // it's a reference to a declaration
            if (VarDecl *VD = dyn_cast<VarDecl>(DRE->getDecl())) {
              // it's a reference to a variable (a local, FuncParam, global, or
              // static data member...)
              llvm::outs() << "variable " << VD->getQualifiedNameAsString()
                           << "\n";
              Modified[Block->getBlockID()].insert(VD);
            }
          }
        }
        // Comment: is this for checking Modifying Expr?
        if (!S.CheckIsNonModifying(E, Sema::NonModifyingContext::NMC_Unknown,
                                   Sema::NonModifyingMessage::NMM_None)) {
          llvm::outs() << "Expr is Modifying...:\n";
          DumpExpression(llvm::outs(), E);
        }
      }
    }

    // Step 8: for each BB with IfStmt terminator, see if the OutputBlock
    // needs to be initialized
    for (const CFGBlock *Block : *Cfg) {
      CFGTerminator CT = Block->getTerminator();
      Stmt *terminatorStmt = CT.getStmt();
      if (terminatorStmt != nullptr && isa<IfStmt>(terminatorStmt)) {
        llvm::outs() << "Terminator is IfStmt:\n";
        // if the IfStmt's condition has the form *e, which means in clang IR:
        // 1. This is a CastExpr that is an lvalue-to-rvalue conversion
        // 2. Whose argument will be a unary operation whose operator is a
        // pointer dereference
        // 3. Whose argument is e

        Expr *E = cast<IfStmt>(terminatorStmt)->getCond();
        // TODO: handle general cases
        // Process BinaryOperator first, e.g. *p == 'a'
        if (E != nullptr && isa<BinaryOperator>(E)) {
          BinaryOperator *BO = dyn_cast<BinaryOperator>(E);
          // left-expr
          if (isa<CastExpr>(BO->getLHS())) {
            // LHS is CastExpr
            ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(BO->getLHS());
            llvm::outs() << "LHS is ImplicitCastExpr:\n";
            // the CastExpr case I encoutered is always an IntegralCast,
            // so check IntegralCast first, e.g.
            // ImplicitCastExpr 0x1ddbeb49100 'int' <IntegralCast>
            // `- ImplicitCastExpr 0x1ddbeb490d0 'char' < LValueToRValue >
            if (CK_IntegralCast == ICE->getCastKind()) {
              ImplicitCastExpr *SICE = dyn_cast<ImplicitCastExpr>(ICE->getSubExpr());
              if (CK_LValueToRValue == SICE->getCastKind()) {
                llvm::outs() << "subexpr is LValueToRValue:\n";
                DumpExpression(llvm::outs(), SICE);
                // ImplicitCastExpr 0x1ebaa74bfc0 'char' <LValueToRValue >
                // `- UnaryOperator 0x1ebaa74bf80 'char' lvalue prefix '*'
                if (isa<UnaryOperator>(SICE->getSubExpr())) {
                  llvm::outs() << "UnaryOperator\n";
                  UnaryOperator *UO = cast<UnaryOperator>(SICE->getSubExpr());
                  if (UO->getOpcode() == UnaryOperatorKind::UO_Deref)
                    // UO->getType()->isCheckedPointerNtArrayType()
                    llvm::outs() << "UO operator is pointer deref:\n";
                  DumpExpression(llvm::outs(), UO);

                  Expr *e = UO->getSubExpr();
                  llvm::outs() << "Expr e:\n";
                  DumpExpression(llvm::outs(), e);

                  // for each vardecl, put the bounds into a canonical form
                  // list<VarDecl *> VarBounds
                  for (auto &VD : VarBounds) {
                    BoundsExpr *DeclaredBounds = VD->getBoundsExpr();
                    if (!DeclaredBounds || DeclaredBounds->isInvalid() ||
                        DeclaredBounds->isUnknown())
                      llvm::outs() << "bad declaredbounds, please check vardecl.\n";

                    llvm::outs() << "Declared Bounds in VarDecl :\n";
                    DumpExpression(llvm::outs(), DeclaredBounds);

					// RangeBoundsExpr(lower, upper+1)
                    BoundsExpr *NormalizedDeclaredBounds = S.ExpandToRange(VD, DeclaredBounds);
                    BoundsExpr *BoundsPlusOne =
                        CreateBoundsPlusOneForNtArray(NormalizedDeclaredBounds->getType());
                    RangeBoundsExpr *RangeValDeclBounds = cast<RangeBoundsExpr>(BoundsPlusOne);

                    llvm::outs() << "Normalized DeclaredBounds():\n";
                    DumpExpression(llvm::outs(), RangeValDeclBounds);

                    Expr *UpperBound = RangeValDeclBounds->getUpperExpr();
                    Expr *UpperBoundSubExpr = nullptr;
                    if (BinaryOperator *BO_bound = dyn_cast<BinaryOperator>(UpperBound))
                      UpperBoundSubExpr = BO_bound->getLHS();

                    llvm::outs() << "Upper Bound SubExpr in VarDecl:\n";
                    DumpExpression(llvm::outs(), UpperBoundSubExpr);

                    // see if the upper bound expr is quivalent to e
                    if (EqualValue(S.Context, e, UpperBoundSubExpr, nullptr)) {
                      llvm::outs() << "e == UpperBoundSubExpr\n";

                      // create the output block map for the "true" edge
                      WidenedBoundsMap *WMap = new WidenedBoundsMap();
                      // TODO: map VarDecl to RangeBoundsExpr(lower, upper+1)
                      (*WMap)[VD] = RangeValDeclBounds;
                      outBoundsMap[Block->getBlockID()].push_back(WMap);

                      llvm::outs() << "outBoundsMap vector size: "
								   << outBoundsMap[Block->getBlockID()].size() << "\n;s
                    }
                  }
                }
              } // end if LValueToRValue
            }
          }
		  // TODO: processsing RHS
          if (isa<CastExpr>(BO->getRHS())) {
            // RHS is CastExpr
            llvm::outs() << "RHS is CastExpr:\n";
            DumpExpression(llvm::outs(), BO->getRHS());
          }
        }

        llvm::outs() << "Dump expression in terminatorStmt:\n";
        DumpExpression(llvm::outs(), E);

        if (E != nullptr && isa<CastExpr>(E)) {
          llvm::outs() << "if this is a CastExpr:\n";
          // llvm::outs() << *E << "\n";
          DumpExpression(llvm::outs(), E);
        }
      }

      // CT.getStmt()->dump(llvm::outs());
    }

	// free two bounds map arrays for the CFG
    for (auto &V : outBoundsMap) {
      for (auto &WM : V) {
        WM->clear();
      }
    }
    for (auto &WM : inBoundsMap) {
      WM->clear();
    }
  ...
  void VisitBinaryOperator(BinaryOperator *E, bool InCheckedScope) {
    ...
    if (!E->isCompoundAssignmentOp() && LHSType->isCheckedPointerPtrType() &&
        RHS->getType()->isCheckedPointerPtrType()) {
      // ptr<T> to ptr<T> assignment, no obligation to infer any bounds for
      // either side
    } else if (LHSType->isCheckedPointerType() || LHSType->isIntegerType() ||
      ...
      if (!LHSTargetBounds->isUnknown()) {
        ...
        CheckBoundsDeclAtAssignment(E->getExprLoc(), LHS, LHSTargetBounds, RHS,
                                    RHSBounds, InCheckedScope);
      }
    }
    ...
    OperationKind OpKind = (E->getOpcode() == BO_Assign) ? OperationKind::Assign
                                                         : OperationKind::Other;
  ...
  void VisitCallExpr(CallExpr *CE, bool InCheckedScope) {
    ...
    for (unsigned i = 0; i < Count; i++) {
      ...
      // We want to check the argument expression implies the desired
      // parameter bounds. To compute the desired parameter bounds, we
      // substitute the arguments for parameters in the parameter bounds
      // expression.
      ...
      if (!ParamBounds && ParamIType)
        ParamBounds =
            S.CreateTypeBasedBounds(nullptr, ParamIType->getType(), true, true);
      ...
      if (ArgBounds->isUnknown()) {
        S.Diag(Arg->getLocStart(), diag::err_expected_bounds_for_argument)
            << (i + 1) << Arg->getSourceRange();
      ...
      BoundsExpr *SubstParamBounds = S.ConcretizeFromFunctionTypeWithArgs(
          const_cast<BoundsExpr *>(ParamBounds), ArgExprs,
      ...
      if (SubstParamBounds->isElementCount() ||
        ...
        if (true /* S.CheckIsNonModifying(Arg,
                              Sema::NonModifyingContext::NMC_Function_Parameter,
                                    Sema::NonModifyingMessage::NMM_Error) */)
          SubstParamBounds =
              S.ExpandToRange(Arg, const_cast<BoundsExpr *>(SubstParamBounds));
        else
          continue;
      }
  ...
  void VisitCastExpr(CastExpr *E, bool InCheckedScope) {
    ...
    if (CK == CK_LValueToRValue && !E->getType()->isArrayType()) {
      bool NeedsBoundsCheck =
          AddBoundsCheck(E->getSubExpr(), OperationKind::Read, InCheckedScope);
    ...
    if (CK == CK_DynamicPtrBounds) {
      ...
      Expr *SubExprAtNewType = BoundsInference(S).CreateExplicitCast(
          E->getType(), CastKind::CK_BitCast, SubExpr, true);
      BoundsExpr *DeclaredBounds = E->getBoundsExpr();
      BoundsExpr *NormalizedBounds =
          S.ExpandToRange(SubExprAtNewType, DeclaredBounds);
      ...
      if (DumpBounds)
        DumpBoundsCastBounds(llvm::outs(), E, DeclaredBounds, NormalizedBounds,
                             SubExprBounds);
    }
  ...
  void VisitUnaryOperator(UnaryOperator *E, bool InCheckedScope) {
    ...
    bool NeedsBoundsCheck =
        AddBoundsCheck(E->getSubExpr(), OperationKind::Other, InCheckedScope);
    if (NeedsBoundsCheck && DumpBounds)
      DumpExpression(llvm::outs(), E);
    return;
  }
  ...
  void VisitVarDecl(VarDecl *D, bool InCheckedScope) {
    ...
    // If there is a scalar initializer, check that the initializer meets the
    // bounds requirements for the variable.  For non-scalar types (arrays,
    // structs, and unions), the amount of storage allocated depends on the
    // type, so we don't to check the initializer bounds.
    Expr *Init = D->getInit();
    if (Init && D->getType()->isScalarType()) {
      ...
      if (InitBounds->isUnknown()) {
        ...
        CheckBoundsDeclAtInitializer(D->getLocation(), D,
                                     NormalizedDeclaredBounds, Init, InitBounds,
                                     InCheckedScope);
      }
      if (DumpBounds)
...
private:
  ...
  void CheckDisallowedFunctionPtrCasts(CastExpr *E) {
    ...
    if (!ToType->isCheckedPointerPtrType() || !ToType->isFunctionPointerType())
      return;
    ...
    if (!S.Context.typesAreCompatible(ToType, NeedleTy,
                                      /* CompareUnqualified=*/false,
                                      /*IgnoreBounds=*/false)) {
      // See if the only difference is that the source is an unchecked pointer
      // type.
  ...
  //
  // This will add the required error messages
  bool CheckValuePreservingCastLikeOp(const UnaryOperator *E,
    ...
    case UO_Deref: {
      ...
      bool between_functions =
          ETy->isFunctionType() && SETy->isFunctionPointerType();

      if (!between_functions) {
        // Add Error Message
        S.Diag(E->getExprLoc(),
               diag::err_cast_to_checked_fn_ptr_can_only_ref_deref_functions)
    ...
    case UO_AddrOf: {
      // This may be more conservative than necessary.
      bool between_functions =
          ETy->isFunctionPointerType() && SETy->isFunctionType();
      if (!between_functions) {
        // Add Error Message
        S.Diag(E->getExprLoc(),
               diag::err_cast_to_checked_fn_ptr_can_only_ref_deref_functions)
...
void Sema::CheckFunctionBodyBoundsDecls(FunctionDecl *FD, Stmt *Body) {
  ...
  std::unique_ptr<CFG> Cfg =
      CFG::buildCFG(nullptr, Body, &getASTContext(), CFG::BuildOptions());
...
#if TRACE_CFG
  llvm::outs() << "Widen bounds for function: " << FD->getName() << "\n";
  Checker.DataFlowAnalysisOnCFG(FD);
#endif
}
...
namespace {
class NonModifiyingExprSema
    : public RecursiveASTVisitor<NonModifiyingExprSema> {
...
public:
  NonModifiyingExprSema(Sema &S, Sema::NonModifyingContext From,
      ...
      : S(S), FoundModifyingExpr(false), ReqFrom(From), Message(Message) {}
  ...
  bool VisitUnaryOperator(UnaryOperator *E) {
    if (E->isIncrementDecrementOp()) {
      addError(E, E->isIncrementOp() ? MEK_Increment : MEK_Decrement);
      FoundModifyingExpr = true;
    }
...
private:
  ...
  void addError(Expr *E, ModifyingExprKind Kind) {
    if (Message != Sema::NonModifyingMessage::NMM_None) {
      for (auto Iter = ModifyingExprs.begin(); Iter != ModifyingExprs.end();
           Iter++) {
        if (*Iter == E)
          return;
      }
      ...
      unsigned DiagId = (Message == Sema::NonModifyingMessage::NMM_Error)
                            ? diag::err_not_non_modifying_expr
                            : diag::note_modifying_expression;
