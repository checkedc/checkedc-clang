...
public:
  ...
  void DataFlowAnalysisOnCFG() {
    assert(Cfg && "expected CFG to exist");

    std::map<CFGBlock *, std::set<CFGBlock *>> OUT;
    std::map<CFGBlock *, std::set<CFGBlock *>> IN;
    std::map<CFGBlock *, std::set<CFGBlock *>> GEN;
    std::map<CFGBlock *, std::set<CFGBlock *>> KILL;

    std::deque<CFGBlock *> changedBlocks;
    ...
    llvm::outs() << "------------------------ BEGIN "
                    "------------------------------------\n";
    llvm::outs() << "CFG NumBlockIDs: " << Cfg->getNumBlockIDs() << "\n";
    llvm::outs() << "Dumping CFG for DataFlowAnalysis: ...\n";
    Cfg->print(llvm::outs(), S.getLangOpts(), true);
    llvm::outs() << "Traversing CFG for DataFlowAnalysis: ...\n";
	// temporary debugging...
    for (const CFGBlock *Block : *Cfg) {
      llvm::outs() << "CFG Block: " << Block->getBlockID() << "\n";
      // put all nodes into the changed set
      changedBlocks.push_back(const_cast<CFGBlock *>(Block));
    ...
    while (!changedBlocks.empty()) {
      // choose a block in changedBlocks, then remove this block from the
      // changed block set
      CFGBlock *currBlock = changedBlocks.front();
      changedBlocks.pop_front();

      // init IN[currBlock] to be empty
      IN[currBlock].clear();

      // calculate IN[currBlock] from predecessors' OUT[p]
      for (CFGBlock::const_pred_iterator I = (*currBlock).pred_begin(),
                                         E = (*currBlock).pred_end();
           I != E; ++I) {
        // IN[curr] = IN[curr] U OUT[p]
        for (auto &p : OUT[*I])
          IN[currBlock].insert(p);
      }

      set<CFGBlock *> OldOut = OUT[currBlock]; // save old OUT[currBlock]

      // TODO: update the bounds for all pointers in expressions in currBlock
      // TODO: OUT[currBlock] = GEN[currBlock] U(IN[currBlock] - KILL[currBlock]);
      // ...
	  // any change to OUT[currBlock] compared to previous value?
      if (!(OldOut == OUT[currBlock])) {
        for (CFGBlock::const_succ_iterator I = (*currBlock).succ_begin(),
                                           E = (*currBlock).succ_end();
             I != E; ++I) {
          // put all successors of currBlock into the changed set
          changedBlocks.push_back(*I);
        }
      }
    } // end while !changedBlocks.empty...
  ...
  // Walk the CFG, traversing basic blocks in reverse post-order.
  ...
  void VisitCallExpr(CallExpr *CE, bool InCheckedScope) {
    ...
    for (unsigned i = 0; i < Count; i++) {
      ...
      // We want to check the argument expression implies the desired parameter
      // bounds. To compute the desired parameter bounds, we substitute the
      // arguments for parameters in the parameter bounds expression.
